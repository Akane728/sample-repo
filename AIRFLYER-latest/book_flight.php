...
td {
  ...
  color: #424242;
        ...
        <?php if(isset($_POST['search_but'])) { 
          ...
          <div class="container-md mt-2">
            ...
            <table class="table table-striped table-bordered table-hover">
              ...
              <tbody>
                <?php
                $sql = 'SELECT * FROM Flight WHERE Destination =? AND 
                    DATE(departure)=? and source=? and DATE(return_dep) is NULL ORDER BY Price';
                $stmt = mysqli_stmt_init($conn);
                mysqli_stmt_prepare($stmt,$sql);                
                mysqli_stmt_bind_param($stmt,'sss',$arr_city,$dep_date,$dep_city);
          ...
          <div class="container-md mt-2">
            <h1 class="display-4 text-center text-light"
              >FLIGHTS <br> <?php echo $dep_city; ?> 
                 to <?php echo $arr_city; ?> <br><?php echo $arr_city; ?> to <?php echo $dep_city; ?> </h1>
            <table class="table table-striped table-bordered table-hover">
              <thead>
                <tr class="text-center">
                  <th scope="col">Aircraft</th>
                  <th scope="col">Departure<br>from <?php echo $dep_city; ?></th>
                  <th scope="col">Arrival<br>to <?php echo $arr_city; ?></th>
                  <th scope="col">Departure<br>from <?php echo $arr_city; ?></th>
                  <th scope="col">Arrival<br>to <?php echo $dep_city; ?></th>
                  <th scope="col">Status</th>
                  <th scope="col">Fare</th>
                  <th scope="col">Buy</th>
                </tr>
              ...
              <tbody>
                <?php
                $sql = 'SELECT * FROM Flight WHERE Destination =? AND 
                    DATE(departure)=? and source=? and DATE(return_dep) is not null ORDER BY Price';
                $stmt = mysqli_stmt_init($conn);
                mysqli_stmt_prepare($stmt,$sql);                
                mysqli_stmt_bind_param($stmt,'sss',$arr_city,$dep_date,$dep_city);
                mysqli_stmt_execute($stmt);
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                  ...
                  <tr class='text-center'>                  
                    ...
                    <td>".$row['departure']."</td>
                    <td>".$row['arrivale']."</td>
                    <td>".$row['return_dep']."</td>
                    <td>".$row['return_arr']."</td>
                    <td>
                      <div>
                          <div class='alert ".$alert." text-center mb-0 pt-1 pb-1' 
                              role='alert'>
	...
	<p class="text-light text-center">&copy; <?php echo date('Y');?> -  Developed By Vera Aguila, Paul Delos Santos, Lindsy Masicat</p>
</footer>