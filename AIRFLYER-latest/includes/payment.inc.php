        ...
        for($i=1;$i<=$passengers;$i++) {
            ...
            if(!mysqli_stmt_prepare($stmt,$sql)) {
                ...
                if($row = mysqli_fetch_assoc($result)) {
                    ...
                    $stmt = mysqli_stmt_init($conn);
                    $sql = 'INSERT INTO Ticket (passenger_id,flight_id,user_id,seat_no,cost,class) VALUES (?,?,?,?,?,?)';            
                    if(!mysqli_stmt_prepare($stmt,$sql)) {
                        header('Location: ../payment.php?error=sqlerror');
                        exit();
                        // echo $stmt;            
                    } else {
                        mysqli_stmt_bind_param($stmt,'iiisis',$pass_id,$flight_id,$_SESSION['userId'],$new_seat,$price,$class);            
                        mysqli_stmt_execute($stmt);
                        // if (mysqli_stmt_execute($stmt)){
                        // // Additional debugging information
                        // echo 'Error: ' . mysqli_stmt_error($stmt);
                        // echo 'Ticket ID: ' . $ticket_id . '<br>';
                        // echo 'Passenger ID: ' . $i . '<br>';
                        // echo 'Flight ID: ' . $flight_id . '<br>';
                        // echo 'User ID: ' . $_SESSION['userId'] . '<br>';
                        // echo 'Seat No: ' . $new_seat . '<br>';
                        // echo 'Cost: ' . $price . '<br>';
                        // echo 'Class: ' . $class . '<br>';
